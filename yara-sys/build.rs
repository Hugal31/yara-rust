...
mod bindings {
    ...
    pub fn add_bindings() {
        let binding_file = format!("yara-4.1.2-{}.rs", env::var("TARGET").unwrap());
        ...
        if binding_path.is_file() {
            fs::copy(binding_path, out_path)
                .expect("Could not copy bindings to output directory");
        }
        else {
            println!("cargo:warning=Bindigs for target=\"{}\" does not exists", env::var("TARGET").unwrap());
            std::process::exit(1);
        }
    }
}
